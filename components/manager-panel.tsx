...
export function ManagerPanel({ tasks }: { tasks: Task[] }) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-3">
      <div className="rounded-2xl border bg-card p-5"><p className="text-sm text-muted-foreground">Tarefas concluídas</p><p className="mt-2 text-3xl font-black">{completed}</p></div>
      <div className="rounded-2xl border bg-card p-5"><p className="text-sm text-muted-foreground">Tarefas pendentes</p><p className="mt-2 text-3xl font-black">{tasks.length - completed}</p></div>
      <div className="rounded-2xl border bg-card p-5"><p className="text-sm text-muted-foreground">Total acompanhado</p><p className="mt-2 text-3xl font-black">{tasks.length}</p></div>
    </div>
    <section className="rounded-2xl border bg-card p-5 shadow-sm">
      <div className="flex items-start gap-3"><FileSpreadsheet className="mt-1 size-5 text-primary" /><div><h2 className="font-extrabold">Importar planilha</h2><p className="mt-1 text-sm text-muted-foreground">Envie um arquivo .xlsx ou .csv com produto, setor, início e término. As tarefas serão encaminhadas aos setores correspondentes.</p></div></div>
      <form action={handleUpload} className="mt-4 flex flex-col gap-3 sm:flex-row"><input name="file" type="file" accept=".xlsx,.csv,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required className="min-w-0 flex-1 rounded-xl border p-2 text-sm" /><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2 font-bold text-primary-foreground"><Upload className="size-4" />Importar planilha</button></form>
