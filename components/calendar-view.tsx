...
export function CalendarView({ tasks, sector, isManager, toggleTaskCompletion, completeTasks, deleteOwnTask, moveOwnTask }: { tasks: TaskWithPromotion[]; sector: string | null; isManager: boolean; toggleTaskCompletion: (formData: FormData) => Promise<void>; completeTasks: (formData: FormData) => Promise<void>; deleteOwnTask: (formData: FormData) => Promise<void>; moveOwnTask: (formData: FormData) => Promise<void> }) {
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-4 px-4 py-6">
      ...
      <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        {isManager ? (selectedTasks.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">Nenhuma verificação agendada.</p> : <ul className="mt-4 space-y-3">{selectedTasks.map((task) => <li key={task.id} className="flex items-start justify-between gap-3 rounded-xl bg-muted p-3"><div><p className="font-bold">{task.productName ?? task.title}</p><p className="mt-1 text-xs text-muted-foreground">{task.type === "start" ? "Início da promoção" : "Término da promoção"} · {formatDate(task.dueDate)}</p><p className="mt-1 text-xs text-muted-foreground">Promoção: {task.title} · Setor: {task.sector}</p></div>{task.completed && <CircleCheck className="mt-1 size-5 shrink-0 text-primary" />}</li>)}</ul>) : (selectedSummaries.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">Nenhuma validação agendada.</p> : <ul className="mt-4 space-y-3">{selectedSummaries.map((summary) => <li key={summary.id} className="rounded-xl bg-muted p-3"><form action={completeTasks} className="mb-3 flex justify-end"><input type="hidden" name="taskIds" value={visibleTasks.filter((task) => task.sector === summary.sector && task.dueDate === summary.dueDate).map((task) => task.id).join(",")} /><button type="submit" className="rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground">Concluir tudo</button></form><details><summary className="flex cursor-pointer list-none items-center justify-between"><div><p className="font-bold">Validação do setor</p><p className="mt-1 text-xs text-muted-foreground">{summary.sector} · {formatDate(summary.dueDate)}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${summary.completed ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{summary.completed ? "Concluído" : "Pendente"}</span></summary><ul className="mt-3 space-y-2 border-t border-border pt-3">{visibleTasks.filter((task) => task.sector === summary.sector && task.dueDate === summary.dueDate).map((task) => <li key={task.id} className="flex items-center justify-between gap-2 rounded-lg bg-card p-2 text-sm"><div><p className="font-semibold">{task.productName ?? task.title}</p><p className="text-xs text-muted-foreground">{task.type === "start" ? "Início" : "Término"} · {task.completed ? "Concluída" : "Pendente"}</p></div><div className="flex flex-wrap justify-end gap-1"><form action={toggleTaskCompletion}><input type="hidden" name="taskId" value={task.id} /><button type="submit" className="rounded-lg bg-primary px-2 py-1 text-xs font-bold text-primary-foreground">{task.completed ? "Reabrir" : "Concluir"}</button></form><form action={deleteOwnTask}><input type="hidden" name="promotionId" value={task.promotionId} /><button type="submit" className="rounded-lg border border-red-200 px-2 py-1 text-xs font-bold text-red-600">Excluir</button></form><form action={moveOwnTask} className="flex gap-1"><input type="hidden" name="promotionId" value={task.promotionId} /><select name="destination" aria-label="Novo setor" className="max-w-28 rounded-lg border border-border px-1 text-xs"><option value="Coleiras">Coleiras</option><option value="Ração gatos">Ração gatos</option><option value="Ração cães">Ração cães</option><option value="Sachês gatos">Sachês gatos</option><option value="Sachês cães">Sachês cães</option><option value="Higiene">Higiene</option><option value="Brinquedos">Brinquedos</option><option value="Areias">Areias</option><option value="Coadjuvantes">Coadjuvantes</option><option value="Farmácia">Farmácia</option></select><button type="submit" className="rounded-lg border border-primary px-2 py-1 text-xs font-bold text-primary">Mover</button></form></div></li>)}</ul></details></li>)}</ul>)}
      </section>
    </div>
  )
}
